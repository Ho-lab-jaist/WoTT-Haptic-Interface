...
def initiate_config():
    ...
    with open('../resources/location.csv') as f:
...
def stop_all_motors():
    ...
    for module_id in modules.keys():
        send_UDP_message(
            COMMAND, modules[module_id]['IP'], modules[module_id]['PORT'], 0, 0)
    print("All Motors Stopped")


def continuous_motion(intensity, duration, delay_time):
    x = 1
    while True:
        stop_all_motors()
        activate_motor(x, intensity, duration)
